  ...
  return (
    <>
      {Array.from({ length: 2 }).map((el, index) => (
        <div
          className={
            "c-modal-registration__social c-modal-registration__social-" + index
          }
          key={"c-modal-registration__social" + index}
        >
  ...
  return (
    <>
      <svg
        ...
        className="c-modal-registration__social-icon"
      >
        <path
...
const Registration = (props) => {
    ...
    if (!values.email) {
      ...
      if (!values.username) {
        ...
        if (!values.password) {
          ...
          errors.confirm = "Required or Password mismatch";
        }
      }
  ...
  const onSubmit = async (
    ...
    await axios
      .post(
        "https://nastya-db-tested.herokuapp.com/auth/registration",
        values,
        headers
      )
      .then((response) => {
        if (response.data) {
          let success = document.getElementById("success-log");
          success.innerHTML = "You are registered in";
          console.log("User added");
        }
      })
      .catch((error) => {
        const errorResponse = error.response.data;
        setErrors({ confirm: errorResponse.message });
  ...
  return (
    <>
      <div
        className={`c-modal-registration ${props.isOpened ? "open" : "close"}`}
      >
        <div className="c-modal-registration__wrapper">
          <div className="c-modal-registration__close-block">
            <div
              className="c-modal-registration__close"
              onClick={props.onModalClose}
            >
              ✕
            </div>
          </div>
          <h3 className="c-modal-registration__title">Sign Up</h3>
          <p className="c-modal-registration__descr">
            ...
            {({
              ...
              return (
                <form
                  ...
                  className="c-modal-registration__form"
                >
                  <label className="c-modal-registration__fullname">
                    Full Name
                    <input
                      className="c-modal-registration__fullname-input"
                      type="text"
                  ...
                  <label className="c-modal-registration__email">
                    Email
                    <input
                      className="c-modal-registration__email-input"
                      type={"email"}
                  ...
                  <label className="c-modal-registration__password">
                    Password
                    <input
                      className="c-modal-registration__password-input"
                      type={"password"}
                      name="password"
                      onChange={handleChange}
                      onBlur={handleBlur}
                  ...
                  <label className="c-modal-registration__confirm-password">
                    Confirm Password
                    <input
                      className="c-modal-registration__confirm-password-input"
                      type={"password"}
                      name="confirm"
                      onChange={handleChange}
                      onBlur={handleBlur}
                  ...
                  <p
                    style={{ textAlign: "center", height: "20px" }}
                    id="success-log"
                    className="c-modal-registration__user-added"
                  ></p>
                  <button className="c-modal-registration__btn " type="submit">
                    Sign Up
                  </button>
                  <p className="c-modal-registration__change">
                    Already have an account?{" "}
                    <span className="c-modal-registration__change-sign-in">
                      Sign in
                    </span>
                  </p>
                </form>
          ...
          <div className="c-modal-registration__divider"></div>
          <div className="c-modal-registration__in-btn">
            <p className="c-modal-registration__in-btn__text">
              Or sign in with
            </p>
            <div className="c-modal-registration__btns">
